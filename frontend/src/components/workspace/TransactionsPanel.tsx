...
export function TransactionsPanel({ onPendingClick }: TransactionsPanelProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
            ...
            <span className="font-bold text-gray-900 dark:text-gray-100">{totalCount}</span>
          </div>
          <div className="flex items-center gap-2 px-4 py-2 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
            <Clock className="h-5 w-5 text-yellow-600" />
            <span className="text-sm text-yellow-700 dark:text-yellow-400">대기</span>
            <span className="font-bold text-yellow-700 dark:text-yellow-400">{pendingCount}</span>
          </div>
        </div>
      ...
      {pendingCount > 0 && filterStatus !== 'pending' && (
        <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg flex items-center justify-between">
          <div className="flex items-center gap-3">
              ...
              <Table>
                ...
                <TableBody>
                  {transactions?.map((transaction) => (
                    <TableRow
                      key={transaction.id}
                      className={selectedIds.has(transaction.id) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}
                    >
                      <TableCell>
                        <SelectCheckbox
                      ...
                      <TableCell>
                        {editingId === transaction.id ? (
                          ...
                          <div
                            ...
                            className={`cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 rounded px-2 py-1 -mx-2 ${
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg space-y-2">
              <div className="flex justify-between">
                <span className="text-sm text-gray-500 dark:text-gray-400">거래일</span>
                <span className="text-sm font-medium">
                  {formatDate(selectedTransaction.transaction_date)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-500 dark:text-gray-400">가맹점</span>
                <span className="text-sm font-medium">
                  {selectedTransaction.merchant_name}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-500 dark:text-gray-400">금액</span>
                <span className="text-sm font-medium">
              ...
              {selectedTransaction.industry && (
                <div className="flex justify-between">
                  <span className="text-sm text-gray-500 dark:text-gray-400">업종</span>
                  <span className="text-sm font-medium">
                    {selectedTransaction.industry}
                  </span>
                </div>
            ...
            {/* 빠른 입력 버튼 */}
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">빠른 입력</p>
